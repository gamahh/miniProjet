...
//	private JButton btSupprimerCategorie;
	...
	public FenetrePrincipale(Controleur p_controleur) {
		controleur = p_controleur;
		this.cns = p_controleur.getCns();
		this.cav = p_controleur.getCav();
		this.cs = p_controleur.getCs();
...
/* tabProduits permet de tester le fonctionnement des fen�tres avec un tableau de noms de produits "en dur"
   Quand l'application fonctionnera, il faudra bien s�r r�cup�rer les noms des produits dans le Catalogue */
		...
		String[] tabProduits = cns.demandeSuppression();

		/* M�me chose pour tabCategories (partie 4) */ 		
//		String[] tabCategories = new String[] {"Bio", "Luxe" };
		
		if (e.getSource() == btAfficher)
			new FenetreAffichage(cs.etatDesStocks());//"ajourd'hui nous allons faire de la programmation en 5 couches");
		if (e.getSource() == btNouveauProduit)
//			new FenetreNouveauProduit(tabCategories);
			new FenetreNouveauProduit(cns);
		if (e.getSource() == btSupprimerProduit)
			new FenetreSuppressionProduit(tabProduits,cns);
//		if (e.getSource() == btNouvelleCategorie)
//			new FenetreNouvelleCategorie();
//		if (e.getSource() == btSupprimerCategorie)
//			new FenetreSuppressionCategorie(tabCategories);
		if (e.getSource() == btAchat)
			new FenetreAchat(tabProduits,cav);
